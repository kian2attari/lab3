...
......................................................................*)
(* Use of anonymous function is not necessary! *)
...
let transcript (enrollments : enrollment list)
               (student : int)
             : enrollment list =
  List.filter (fun searchStudent -> searchStudent.id = student) enrollments ;;
...
let ids (enrollments: enrollment list) : int list =
  List.sort_uniq (compare) (List.map (fun studentSearch
                                       -> studentSearch.id) enrollments) ;;
...
let schoolName (enrollments : enrollment list) : string list =
...
let verify (enrollments : enrollment list) : bool =
  List.for_all (fun x -> List.length x = 1)
    (List.map (fun studentSearch -> schoolName
                  (transcript enrollments studentSearch)) (ids enrollments)) ;;
...
......................................................................*)
(*  Two separate expression as a tuple output *)

let rec partition (checker : 'a -> bool) (lst : 'a list) : 'a list * 'a list =
  List.filter checker lst, List.filter (fun x -> not (checker x)) lst
...
let apply (inputFunction : 'a -> 'b) (argument : 'a) : 'b =
